...
type PluginApi = {
  ...
  registerGatewayMethod: (name: string, handler: (ctx: unknown) => void) => void;
};
...
function getCfg(api: PluginApi): Partial<HueConfig> {
  ...
  return (entry?.config ?? {}) as Partial<HueConfig>;
}

function buildClient(cfg: Partial<HueConfig>): HueBridgeClient {
...
export default function register(api: PluginApi) {
  api.registerGatewayMethod(`${PLUGIN_ID}.status`, (ctx: unknown) => {
    const respond = (ctx as { respond?: (ok: boolean, data: unknown) => void })?.respond;
  ...
  api.registerTool(
    {
      ...
      async execute(_requestId: string, params: Record<string, unknown>) {
        ...
        return asTextContent({
          ...
          transitionMs: transitionMs ?? cfg.defaultTransitionMs ?? null,
        });
      },
    },
    { optional: true },
